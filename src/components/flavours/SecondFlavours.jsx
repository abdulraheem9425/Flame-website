...
const DipsComponent = () => {
  return (
    <section className="bg-red-700 text-white py-24 px-4 sm:px-8 text-center overflow-hidden">
      
      {/* Animated Heading */}
      <motion.h2
        className="text-4xl sm:text-5xl font-extrabold text-yellow-400 mb-12"
      ...
      <div className="flex justify-center flex-wrap gap-8 sm:gap-10 mb-16">
        {dips.map((dip, idx) => (
          ...
          >
            <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-full bg-white shadow-xl flex items-center justify-center relative z-10 hover:scale-105 transition-transform duration-300">
              <div className={`w-24 h-24 sm:w-32 sm:h-32 rounded-full ${dip.bgColor} flex items-center justify-center`}>
                <span className={`text-center text-sm sm:text-md font-extrabold leading-tight px-2 ${dip.textColor}`}>
                  {dip.name}
                </span>
              </div>
            </div>
            <div className="w-4 h-4 sm:w-5 sm:h-5 bg-white rotate-45 -mt-2 z-0" />
          </motion.div>
        ))}
      </div>
